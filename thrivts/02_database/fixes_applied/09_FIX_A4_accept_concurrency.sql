-- ============================================================================
-- A4 — prevent over-allocation on concurrent accepts (+ carries A2's fixes)
--
-- Race: two buyers/tabs accepting different bids on the SAME requirement could
-- both read the committed-quantity total before either commits, both pass the
-- remaining-qty guard, and over-allocate. Fix: lock the requirement row with
-- FOR UPDATE at load time, so a second accept on that requirement waits for the
-- first to commit, then re-reads the (now-updated) total and rejects an over-fill.
-- Accepts on different requirements lock different rows and don't block.
--
-- This file also INCLUDES A2 (duplicate seller-notification trigger dropped +
-- step-10 silent catch now logs), so running it alone is sufficient; running it
-- after A2 simply re-applies the same function with the lock added (safe).
-- ============================================================================
