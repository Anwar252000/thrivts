...
import { useGetMessageThreadsQuery, useGetMyProfileQuery, buyerApi } from '@/features/buyer/buyerApi'
import { cn } from '@/lib/utils'

const NAV_ITEMS = [
  { to: '/buyer', label: 'Dashboard', end: true },
  { to: '/buyer/post-requirement', label: 'Post requirement' },
  { to: '/buyer/requirements', label: 'My requirements' },
...
/** The buyer portal's chrome — a top nav bar with pill links (buyer.html's .app-nav), unlike
 * admin's dark sidebar. Gates on approval status: Pending/Rejected/Suspended buyers see a status
 * screen instead of the nav+app, matching buyer.html's separate pending/rejected views. */
...
function ApprovedShell() {
  ...
  const handleSync = () => {
    dispatch(buyerApi.util.invalidateTags(['Dashboard', 'Requirement', 'Deal', 'MessageThread']))
  }
        ...
        <div className="mx-auto flex max-w-[1280px] items-center gap-5 px-5 py-3">
                ...
                {({ isActive }) => (
                  <>
                    ...
                    {item.to === '/buyer/messages' && unreadCount > 0 && (
                      <span className="ml-1.5 inline-flex size-[16px] items-center justify-center rounded-full bg-[var(--color-danger)] text-[0.62rem] font-bold text-white">
                        {unreadCount}
                      </span>
                    )}
                  </>
          ...
          <div className="flex shrink-0 items-center gap-2.5">
            ...
            >
              ...
              <span className="max-w-[140px] truncate text-sm font-medium">{user?.fullName ?? 'Buyer'}</span>
            </button>
            <button
...
function PendingScreen() {
  ...
  return (
    <StatusShell icon={Clock} tone="warning" title="Application received" body="Thanks for applying. Our team reviews every application — usually within 24 hours. We'll email you the moment your account is approved.">
      {profile && (
        <div className="mt-2 grid grid-cols-2 gap-3 rounded-[var(--radius-sm)] bg-[var(--color-sage-mist)] p-4 text-left text-sm">
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-[var(--color-ink-faint)]">Country</p>
  ...
  return (
    <StatusShell icon={XCircle} tone="danger" title="Application not approved" body="Unfortunately we weren't able to approve your application at this time.">
      ...
      <p className="text-xs text-[var(--color-ink-faint)]">
        If you believe this is a mistake, email <a className="underline" href="mailto:support@thrivts.com">support@thrivts.com</a>.
      </p>
    </StatusShell>
  )
}
